    ...
    <div className="bg-white">
      <div className="max-w-7xl mx-auto px-4 py-12 sm:px-6 lg:px-8">
            ...
            <div className="bg-gray-100 rounded-3xl overflow-hidden aspect-square flex items-center justify-center relative group border border-gray-100">
              ...
              {product.customizable && currentImageIndex === 0 && (
                <Link 
                  ...
                  className="absolute bottom-6 right-6 bg-white/90 backdrop-blur px-4 py-2 rounded-full text-sm font-bold text-indigo-600 shadow-lg flex items-center gap-2 z-20 hover:scale-105 transition"
                >
              ...
              {gallery.length > 1 && (
                <>
                  <button 
                    onClick={prevSlide} 
                    className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/80 p-2 rounded-full text-gray-800 opacity-0 group-hover:opacity-100 transition"
                  >
                    <ChevronLeft size={24}/>
                  </button>
                  <button 
                    onClick={nextSlide} 
                    className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/80 p-2 rounded-full text-gray-800 opacity-0 group-hover:opacity-100 transition"
                  >
                  ...
                  <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
                    {gallery.map((_, idx) => (
                      <button 
                        ...
                        className={`w-2 h-2 rounded-full transition-all shadow-sm ${currentImageIndex === idx ? 'bg-indigo-600 w-4' : 'bg-white/60'}`}
                      />
                    ))}
                  </div>
                </>
          ...
          <div>
            ...
            <div className="space-y-4">
              ...
              {product.customizable && (
                <Link 
                  to={`/customize/${product.id}`} 
                  className="w-full border-2 border-dashed border-indigo-200 text-indigo-600 py-3 rounded-xl font-bold text-center hover:bg-indigo-50 transition flex items-center justify-center gap-2"
                >
                  <Wand2 size={18} /> Want to change colors or finish? Customize this item
                </Link>
              )}
            </div>
          </div>
        ...
        {relatedProducts.length > 0 && (
          <section className="py-16 -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8 bg-gray-50 border-t border-gray-100">
